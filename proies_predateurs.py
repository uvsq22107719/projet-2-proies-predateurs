...
def tour_suivant():
    ...
    for i in range(1, N + 1): # Pour chaque liste (ex : [0, 0, ["Proie", 5, 3], ["Proie", 2, 3], 0, ["Prédateur", 15, 12], 0])
        for j in range(1, N + 1): # Pour chaque élément dans la liste (ex : ["Proie", 5, 3])
            if type(config[i][j]) == list and config[i][j][0] == "Proie" and config[i][j][2] < Epro: # Si c'est une proie et que celle-ci a une énergie inférieure à Epro
    ...
    for i in range(1, N + 1):
        for j in range(1, N + 1):
            if type(config[i][j]) == list and config[i][j][0] == "Proie" and config[i][j][-1] != "Reproduit" and config[i][j][2] >= Epro: # Seulement si c'est une proie, qu'elle ne s'est pas déjà reproduite pendant ce tour et que son niveau d'énergie est maximal (égal à Epro)
    ...
    for i in range(1, N + 1):
        for j in range(1, N + 1):
            if type(config[i][j]) == list and config[i][j][0] == "Prédateur" and config[i][j][-1] != "Reproduit" and config[i][j][2] >= Erepro: # Seulement si c'est un prédateur, qu'il ne s'est pas déjà reproduit pendant ce tour et que son niveau d'énergie est supérieur ou égal au niveau d'énergie Erepro nécessaire pour pouvoir se reproduire
                config[i][j].append("Reproduit") # Ajouter le terme "Reproduit" à la fin de la liste pour éviter qu'il se reproduise deux fois dans le même tour
                r = True # Variable pour arrêter la boucle
                while r: # Tant que r == True
                    i = rd.randint(1, N + 1) # Abscisse aléatoire
                    j = rd.randint(1, N + 1) # Ordonnée aléatoire
                    if config[i][j] == 0: # Si la case à ces coordonnées est vide
                        config[i][j] = ["Prédateur", Apro, Epre] # Ajouter un prédateur
    ...
    for i in range(1, N + 1):
        for j in range(1, N + 1):
            if type(config[i][j]) == list and config[i][j][0] == "Prédateur" and config[i][j][-1] != "Déplacé" : # Seulement si c'est un prédateur et qu'il n'a pas déjà effectué un déplacement pendant ce tour
                case = direction(i, j, "Proie") # Retourner une case adjacente aléatoire où se situe une proie
                if case == 0: # S'il n'y a pas de proie à côté
                    case = direction(i, j, 0) # Cherche une case vide adjacente aléatoire
                    if case == 0: # S'il n'y en a pas non plus
                        break # Annuler la boucle
                    elif case == 1: # Si la case 1 est vide
                        config[i-1][j-1] = config[i][j][:] # Copie de la liste sur la nouvelle position
                        config[i][j] = 0 # Suppression de la liste sur l'ancienne position
                        config[i-1][j-1].append("Déplacé") # Ajout du terme "Déplacé" à la fin de la liste pour éviter de déplacer le même animal 2 fois dans le même tour
                    elif case == 2:
                        config[i-1][j] = config[i][j][:]
                        config[i][j] = 0
                        config[i-1][j].append("Déplacé")
                    ...
                    config[i-1][j-1] = config[i][j][:] # Copie de la liste sur la nouvelle position
                    config[i][j] = 0 # Suppression de la liste sur l'ancienne position
                    config[i-1][j-1].append("Déplacé") # Ajout du terme "Déplacé" à la fin de la liste pour éviter de déplacer le même animal 2 fois de suite
